...
export async function init() {
    const canvas = required(document.getElementById("canvas")) as HTMLCanvasElement
...
function startAnimation(renderer: Renderer, gpuContext: GPUCanvasContext) {
...
class Renderer {
    ...
    constructor(private gpuDevice: GPUDevice, format: GPUTextureFormat, texture3D: GPUTexture) {
    ...
    private createRenderPipeline(groupLayout: GPUBindGroupLayout, format: GPUTextureFormat): GPURenderPipeline {
    ...
    private createBindGroup(textureGroupLayout: GPUBindGroupLayout, texture3D: GPUTexture): GPUBindGroup {
    ...
    async setAspectRatio(value: number) {
        this.gpuDevice.queue.writeBuffer(this.uniforms, 0, new Float32Array([value]))
    }

    renderTo(texture: GPUTexture, elapsedTime: number) {
...
class Texture3DComputer {
    ...
    private textureStorageGroup: GPUBindGroup

    constructor(private gpuDevice: GPUDevice, texture3D: GPUTexture) {
    ...
    private createTextureStorageGroupLayout(gpuDevice: GPUDevice) {
    ...
    private createComputePipeline(textureStorageGroupLayout: GPUBindGroupLayout) {
    ...
    private createTextureStorageGroup(textureStorageGroupLayout: GPUBindGroupLayout, texture3D: GPUTexture): GPUBindGroup {
        return this.gpuDevice.createBindGroup({
...
const renderingShaderCode = /*wgsl*/`
    ...
    fn f_main(position: Position) -> Fragment {
        ...
        let p = 0.5 * (position.model_position.xyz + 1.0);
        // Comment the following line, and uncomment the one after to see how the object should be rendered with colors
        return Fragment(textureSample(texture, texture_sampler, p.xy, u32(f32(layers) * p.z)));
        // return Fragment((position.model_position + 1.0) * vec4(vec3(0.5), 1.0));
    }
...
const texture3DComputeShaderCode = /*wgsl*/`
    @group(0)
    @binding(0)
    var texture_storage: texture_storage_2d_array<rgba16float, write>;

    @compute
    @workgroup_size(4, 4, 4)
    fn c_main(@builtin(global_invocation_id) global_invocation_id: vec3<u32>) {
        let pos = vec3<f32>(global_invocation_id) / 15.0;
        let color = vec4(pos, 1.0);
        textureStore(texture_storage, global_invocation_id.xy, global_invocation_id.z, color);
    }
