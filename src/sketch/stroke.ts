...
export class TesselationShader {
    ...
    tesselationGroup(strokeAttributes: StrokeAttributes, inputStrokePoints: InputPoint[], segmentsCount: number): TesselationGroup {
        return this.pipeline.bindGroup("tesselation", this.pipeline.device.dataBuffers({
            ...
            outputStrokePoints: { usage: ["STORAGE"], size: outputPointStruct.paddedSize * segmentsCount },
        }))
    }

    tesselate(tesselation: TesselationGroup) {
        const segmentsCount = Math.ceil(tesselation.entries.outputStrokePoints.baseResource().size / outputPointStruct.paddedSize);
...
export type InputPoint = gpu.DataTypeOf<typeof inputPointStruct>
export const inputPointStruct = gpu.struct({
    position: gpu.f32.x2,
    linear: gpu.f32.x2,
}) 

export type OutputPoint = gpu.DataTypeOf<typeof outputPointStruct>
export const outputPointStruct = gpu.struct({
    left: gpu.f32.x2,
    right: gpu.f32.x2,
    linear: gpu.f32.x2,
}) 
...
export const outputStrokePointsBinding = gpu.storage("read_write", outputPointStruct)
...
const tesselationShader = /* wgsl */ `
    ...
    @group(0) @binding(1)
    var<storage, read> input_stroke_points: array<InputPoint>;
    @group(0) @binding(2)
    var<storage, read_write> output_stroke_points: array<OutputPoint>;
    ...
    fn compute_point(index: u32, points_count: u32) -> OutputPoint {
    ...
    fn output_point(p: Point) -> OutputPoint {
    ...
    fn input_point(i: i32) -> InputPoint {
