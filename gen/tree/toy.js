...
function doInit() {
    ...
    Gear.Flow.from(depthInc.clickPos.map(() => +1), depthDec.clickPos.map(() => -1)).reduce((inc, depth) => Math.max(Math.min(8, inc + depth), 1), 5).branch(flow => flow.map(depth => depth.toString()).to(Gear.text("depth")), flow => flow.map(depth => {
