...
export function charsOtherThan(c) {
...
export function computeOverlaps(...sets) {
    const result = [];
    const limits = sets.flatMap((set, i) => limitsOfRanges(set.ranges).map(limit => utils.pair(i, limit))).sort(utils.comparing(pair => pair.value, compareLimits));
    let ids = [];
    let lastLimit = {
        value: -1,
        upper: true
