...
export class SyncBuffer {
    ...
    static instances(device, descriptors, labelPrefix) {
        return utl.replaceValues(descriptors, (descriptor, key) => SyncBuffer.instance(device, utl.withLabel(descriptor, labelPrefix, key)));
    }
    static instance(device, descriptor) {
...
export class DataBuffer extends AbstractBuffer {
        ...
        return {
            asData: async (data, dataOffset = 0) => {
                ...
                try {
                    ...
                    return await temp.get().asData(data, dataOffset);
                }
                finally {
                    temp.destroy();
                }
    ...
    static instances(device, descriptors, labelPrefix) {
        return utl.replaceValues(descriptors, (descriptor, key) => DataBuffer.instance(device, utl.withLabel(descriptor, labelPrefix, key)));
    }
    static instance(device, descriptor) {
...
export class ReadBuffer extends AbstractBuffer {
    ...
    get(bufferSegment = {}) {
        return {
            asData: async (data, dataOffset = 0) => {
                ...
                const src = await this.mapper.request(bufferOffset, size);
    ...
    static instances(device, descriptors, labelPrefix) {
        return utl.replaceValues(descriptors, (descriptor, key) => ReadBuffer.instance(device, descriptor, utl.label(labelPrefix, key)));
    }
    static instance(device, size, label) {
...
export class WriteBuffer extends AbstractBuffer {
    ...
    set(bufferSegment = {}) {
        return {
            fromData: async (data, dataOffset = 0) => {
                const bufferOffset = bufferSegment.offset ?? 0;
                const size = bufferSegment.size ?? Math.min(this.size - bufferOffset, data.byteLength - dataOffset);
                const dst = await this.mapper.request(bufferOffset, size);
    ...
    static instances(device, descriptors, labelPrefix) {
        return utl.replaceValues(descriptors, (descriptor, key) => WriteBuffer.instance(device, descriptor, utl.label(labelPrefix, key)));
    }
    static instance(device, data, label) {
    ...
    return {
        from: (srcBuffer, srcOffset = 0) => ({
            to: (dstBuffer, dstOffset = 0) => {
                doCopy(srcBuffer, srcOffset, dstBuffer, dstOffset, segmentSize);
            }
        }),
        to: (dstBuffer, dstOffset = 0) => ({
            from: (srcBuffer, srcOffset = 0) => {
...
function doCopy(srcBuffer, srcOffset, dstBuffer, dstOffset, size = Math.min(srcBuffer.size - srcOffset, dstBuffer.size - dstOffset)) {
...
class BufferMapper {
    constructor(buffer, mode) {
        ...
        this.unmappedPromise = Promise.resolve();
        this.unmappedResolve = () => { };
    }
    async request(offset, size) {
        await this.unmappedPromise;
        const validOffset = lowerMultipleOf(8, offset);
        const offsetCorrection = offset - validOffset;
        const validSize = upperMultipleOf(4, size + offsetCorrection);
        const buffer = await this.doRequest(validOffset, validSize);
        return new Uint8Array(buffer, offsetCorrection, size);
    }
    async doRequest(offset, size) {
        this.range = [Math.min(this.range[0], offset), Math.max(this.range[1], offset + size)];
        try {
            await this.map();
            return this.buffer.getMappedRange(offset, size).slice();
        }
        finally {
            this.unmap();
        }
    ...
    async map() {
        this.referenceCount++;
        await (this.referenceCount > 1 ? this.mappedPromise : this.mappedPromise = this.scheduleMapping());
    }
    unmap() {
        this.referenceCount--;
    ...
    async scheduleMapping() {
        await utl.later();
        this.unmappedPromise = new Promise(resolve => this.unmappedResolve = resolve).then(() => this.reset());
        await this.buffer.mapAsync(this.mode, this.range[0], this.range[1] - this.range[0]);
    }
    reset() {
        this.referenceCount = 0;
        this.range = [this.buffer.size, 0];
