...
export class Device {
    constructor(_wrapped, deviceDescriptor) {
    ...
    async shaderModules(descriptors, labelPrefix) {
        const result = {};
    ...
    async remoteShaderModule(label, relativePath, templateFunction = s => s, basePath = "/shaders") {
    ...
    async inMemoryShaderModule(label, rawShaderCode, templateFunction = s => s) {
    ...
    dataBuffers(descriptors, labelPrefix) {
        const result = {};
        for (const k in descriptors) {
    ...
    dataBuffer(descriptor) {
        return new buf.DataBuffer(this, descriptor);
    }
    readBuffers(descriptors, labelPrefix) {
        const result = {};
        for (const k in descriptors) {
    ...
    readBuffer(size, label) {
        return new buf.ReadBuffer(this, size, label);
    }
    writeBuffers(descriptors, labelPrefix) {
        const result = {};
        for (const k in descriptors) {
    ...
    writeBuffer(data, label) {
        return new buf.WriteBuffer(this, data, label);
    }
    syncBuffers(descriptors, labelPrefix) {
        const result = {};
        for (const k in descriptors) {
    ...
    syncBuffer(descriptor) {
        return buf.SyncBuffer.create(this, descriptor);
    }
    groupLayouts(descriptors, labelPrefix) {
        const result = {};
        for (const k in descriptors) {
    ...
    groupLayout(descriptor) {
        return new grp.BindGroupLayout(this, descriptor);
    }
    pipelineLayouts(descriptors, labelPrefix) {
        const result = {};
        for (const k in descriptors) {
    ...
    static async instance(deviceDescriptor = {}) {
...
async function deviceAndDescriptor(deviceDescriptor) {
...
async function requestAdapter(gpu, xrCompatible) {
    const adapter = utl.required(await utl.timeOut(gpu.requestAdapter({ xrCompatible }), 5000, "GPU Adapter"), () => "No suitable GPU Adapter was found!");
...
async function requestDevice(adapter, deviceDescriptor) {
    const device = utl.required(await utl.timeOut(adapter.requestDevice(deviceDescriptor), 5000, "GPU Device"), () => "Failed to create a GPU Device!");
