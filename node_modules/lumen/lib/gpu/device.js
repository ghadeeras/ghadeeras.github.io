...
async function requestAdapter(gpu, xrCompatible) {
    const adapter = required(await timeOut(gpu.requestAdapter({ xrCompatible }), 5000, "GPU Adapter"), () => "No suitable GPU Adapter was found!");
...
async function requestDevice(adapter, deviceDescriptor) {
    const device = required(await timeOut(adapter.requestDevice(deviceDescriptor), 5000, "GPU Device"), () => "Failed to create a GPU Device!");
