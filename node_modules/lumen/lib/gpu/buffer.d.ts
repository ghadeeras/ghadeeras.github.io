...
import * as typ from "./types.js";
import { Device } from "./device.js";
export type SyncBuffers<D extends DataBufferDescriptors> = utl.ReplaceValues<D, SyncBuffer>;
export declare class SyncBuffer implements utl.Resource<DataBuffer> {
    readonly gpuBuffer: DataBuffer;
    readonly cpuBuffer: DataView;
    private dirtyRange;
    private constructor();
    get label(): string;
    baseResource(): DataBuffer;
    asBindingResource(binding?: Omit<GPUBufferBinding, "buffer">): GPUBindingResource;
    get<T>(element: typ.Element<T>): T;
    set<T>(element: typ.Element<T>, value: T): void;
    private dirty;
    private clean;
...
export interface SourceBuffer extends BaseBuffer {
    copy(segment?: typ.DataSegment): DestinationClause<void>;
    set(segment?: typ.DataSegment): BufferWriter;
}
export interface DestinationBuffer extends BaseBuffer {
    copy(segment?: typ.DataSegment): SourceClause<void>;
    get(segment?: typ.DataSegment): BufferReader;
}
export declare class DataBuffer extends AbstractBuffer implements utl.Resource<DataBuffer>, SourceBuffer, DestinationBuffer {
    readonly descriptor: DataBufferDescriptor;
    private constructor();
    baseResource(): DataBuffer;
    copy(segment?: typ.DataSegment): DestinationClause<void> & SourceClause<void>;
    setData(data: DataView): Promise<void>;
    set(bufferSegment?: typ.DataSegment): BufferWriter;
    get(bufferSegment?: typ.DataSegment): BufferReader;
...
export type BufferUsage = utl.StrictExclude<(keyof GPUBufferUsage), "MAP_READ" | "MAP_WRITE" | "COPY_SRC" | "COPY_DST">[];
export type DataBuffers<D extends DataBufferDescriptors> = utl.ReplaceValues<D, DataBuffer>;
...
export declare class ReadBuffer extends AbstractBuffer implements DestinationBuffer {
    private mapper;
    private constructor();
    copy(bufferSegment?: typ.DataSegment): SourceClause<void>;
    get(bufferSegment?: typ.DataSegment): BufferReader;
...
export type ReadBuffers<D extends ReadBufferDescriptors> = utl.ReplaceValues<D, ReadBuffer>;
...
export declare class WriteBuffer extends AbstractBuffer implements SourceBuffer {
    ...
    private mapper;
    private constructor();
    copy(bufferSegment?: typ.DataSegment): DestinationClause<void>;
    set(bufferSegment?: typ.DataSegment): BufferWriter;
...
export type WriteBuffers<D extends WriteBufferDescriptors> = utl.ReplaceValues<D, WriteBuffer>;
